...
const CurrencyHistory = () => {
  ...
  const fetchHistoryData = useCallback(async () => {
    try {
      ...
      const oldestPossibleDate = moment()
        .subtract(7, "days")
        .format("YYYY-MM-DD");
      const response: AxiosResponse = await axios.get(
        `https://free.currconv.com/api/v7/convert?q=${fromCurrency}_${toCurrency}&compact=ultra&date=${oldestPossibleDate}&endDate=${actualDate}&apiKey=3b385138a6bddd2a8b03`
      );
      let id = 0;
      const historyResults = Object.entries(
      ...
      {historyData.length ? (
        <Fragment>
          <div className="HistoryData">
            ...
            <span className="AfterConvertion">
            ...
            {historyData.map((item: HistoryData) => (
              <Fragment key={item.id}>
                ...
                <span key={item.id} className="AfterConvertion">
                  {item.value}
                </span>
              </Fragment>
            ))}
        ...
        <div className="LackOfHistory">{"Brak historii konwersji walut"}</div>
      )}
